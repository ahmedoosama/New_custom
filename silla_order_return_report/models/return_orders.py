...
class PosOrderReport(models.Model):
    ...
    @api.model_cr
    def init(self):
        tools.drop_view_if_exists(self._cr, 'report_pos_return_order')
        self._cr.execute("""
            CREATE OR REPLACE VIEW report_pos_return_order AS (
                SELECT
                    MIN(l.id) AS id,
                    COUNT(*) AS nbr_lines,
                ...
                GROUP BY
                    ...
                    s.worker_id
                HAVING
                    SUM(l.qty * u.factor) != 0 and s.is_return_order = true
            )
        """)
